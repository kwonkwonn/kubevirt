...
var _ = Describe("AccessCredentials", func() {
	...
	Context("path traversal defense", func() {
		...
		It("should not follow symlinks when reading user passwords from secret directory", func() {
			secretDir := filepath.Join(tmpDir, "some-secret-access-cred")
			Expect(os.Mkdir(secretDir, 0o755)).To(Succeed())

			sensitiveFile := filepath.Join(tmpDir, "sensitive-data")
			Expect(os.WriteFile(sensitiveFile, []byte("sensitive-password"), 0o600)).To(Succeed())
			Expect(os.Symlink(sensitiveFile, filepath.Join(secretDir, "malicious-user"))).To(Succeed())

			dir, err := safepath.JoinAndResolveWithRelativeRoot(secretDir)
			Expect(err).NotTo(HaveOccurred())
			passMap := map[string]string{}
			err = readAndAddPasswordsFromDirectory(dir, passMap)
			Expect(err).To(HaveOccurred(), "symlink traversal outside secret directory must be rejected")
		})
	})
